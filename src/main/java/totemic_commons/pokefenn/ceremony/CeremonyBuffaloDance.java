...
public class CeremonyBuffaloDance extends Ceremony
{
    ...
    public void effect(World world, int x, int y, int z)
    {
        ...
        for(EntityLiving entity : world.selectEntitiesWithinAABB(EntityLiving.class, aabb, CeremonyBuffaloDance::isValidTarget))
        {
            if(buffalos < 2)
            {
                buffalos++;
    ...
    private static boolean isValidTarget(Entity entity)
    {
        ...
        return ConfigurationSettings.BUFFALO_DANCE_TARGETS.contains(entityId);
    }
}
