...
public class PlayerRender
{
    ...
    public void onPlayerRender(RenderPlayerEvent.Post event)
    {
        ...
        if(ljfaUUID.equals(player.getUniqueID()) && annaSkinId.equals(player.getLocationSkin().getResourcePath()))
        {
            ...
            float pitchZ = (float)Math.toDegrees(event.renderer.getMainModel().bipedHead.rotateAngleZ);

            GlStateManager.pushMatrix();
            if(player.isSneaking())
                GlStateManager.translate(0, -0.25F, 0);
            GlStateManager.translate(event.x, event.y + 1.4071875F, event.z); // 1.501 * 0.9375
