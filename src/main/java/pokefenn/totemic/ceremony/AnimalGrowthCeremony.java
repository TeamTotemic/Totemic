...
public enum AnimalGrowthCeremony implements CeremonyInstance {
    ...
    public void effect(Level level, BlockPos pos, CeremonyEffectContext context) {
        if(context.getTime() % 20 == 0) {
            ...
            growAnimals(level, aabb);
            hatchChickenEggs(level, aabb);
            hatchTurtleEggs(level, pos);
        }
    }
        ...
        if(!level.isClientSide) {
            ...
            .forEach(egg -> {
                if(level.random.nextInt(4) == 0) {
                    ...
                    chicken.moveTo(egg.getX(), egg.getY(), egg.getZ(), level.random.nextFloat() * 360.0F, 0.0F);
    ...
    private static void hatchTurtleEggs(Level level, BlockPos pos) { //TODO: Look into how forEachBlockIn can be called less frequently
