...
public enum RegistryApiImpl implements RegistryAPI {
    ...
    public static void createRegistries(NewRegistryEvent event) {
        ...
        woodTypes = event.create(new RegistryBuilder<TotemWoodType>().setType(TotemWoodType.class).setName(WOOD_TYPE_REGISTRY.location()).setDefaultKey(Totemic.resloc("oak")).disableSaving().disableSync());
        totemCarvings = event.create(new RegistryBuilder<TotemCarving>().setType(TotemCarving.class).setName(TOTEM_CARVING_REGISTRY.location()).setDefaultKey(Totemic.resloc("none")).disableSaving().disableSync());
