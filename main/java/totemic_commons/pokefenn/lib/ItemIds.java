package totemic_commons.pokefenn.lib;

public class ItemIds
{


    public static int CHLOROPHYLL_CRYSTAL;
    public static int TOTEM_WHITTLING_KNIFE;
    public static int TOTEMIC_STAFF;
    public static int BUCKET_CHLOROPHYLL;
    public static int VENUS_FLY_TRAP_SEED;
    public static int TOTEM_BEADS;
    public static int SUB_ITEMS;
    public static int BOTTLE_CHLOROPHYLL;
    public static int TOTEMS;
    public static int BLAZING_CHLOROPHYLL_CRYSTAL;
    public static int INFUSED_TOTEMIC_STAFF;
    public static int TOTEMPEDIA;

    public static int TOTEM_WHITTLING_KNIFE_DEFAULT = 6001;
    public static int TOTEMIC_STAFF_DEFAULT = 6002;
    public static int CHLOROPHYLL_CRYSTAL_DEFAULT = 6003;
    public static int BUCKET_CHLOROPHYLL_DEFAULT = 6004;
    public static int VENUS_FLY_TRAP_SEED_DEFAULT = 6005;
    public static int TOTEM_BEADS_DEFAULT = 6006;
    public static int SUB_ITEMS_DEFAULT = 6007;
    public static int BOTTLE_CHLOROPHYLL_DEFAULT = 6008;
    public static int BLAZING_CHLOROPHYLL_CRYSTAL_DEFAULT = 6009;
    public static int TOTEMS_DEFAULT = 6010;
    public static int INFUSED_TOTEMIC_STAFF_DEFAULT = 6011;
    public static int TOTEMPEDIA_DEFAULT = 6012;

    public static int TOTEM_WOOD_WAND_CORE;
    public static int TOTEM_WOOD_WAND_CORE_DEFAULT = 6020;
    public static int CHLOROPHYLL_CRYSTAL_WAND_CAP;
    public static int CHLOROPHYLL_CRYSTAL_WAND_CAP_DEFAULT = 6022;


}
