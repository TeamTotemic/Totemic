...
public class TotemEffectBat implements ITotemEffect
{
    ...
    {
        if (totem.getWorldObj().getWorldTime() % 40L == 0L)
        {
            ...
            {
                for (Entity entityArea : EntityUtil.getEntitiesInRange(totem.getWorldObj(), totem.xCoord, totem.yCoord, totem.zCoord, 10, 10))
                {
                    if (entityArea instanceof EntityPlayer)
                    {
                        ...
                        totem.getWorldObj().updateEntity(entityArea);
